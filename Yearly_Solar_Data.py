...
def TimeoutHTTPAdapter(url_nw, url_ne, url_sw, url_se, url_c, url_v):
    ...
    print("response fetched", response_sw, "\n")
...
#,
def download_zip(download_url_nw,download_url_ne, download_url_sw, download_url_se,download_url_c, download_url_v, dest={destination}):
    ...
    if start_year.isdigit() and end_year.isdigit() and interval.isdigit():
        if int(start_year) >= 2018:
            ...
            if response_sw.status_code == 200 and response_ne.status_code == 200 \
                ...
                time.sleep(2400)
                TimeoutHTTPAdapter(download_url_sw) 
                download_zip(download_url_sw,destination)
        else:
